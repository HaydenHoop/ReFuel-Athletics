    ...
    <header className="w-full sticky top-0 z-20 bg-gray-50 border-b border-gray-200">
      <div className="max-w-5xl mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          ...
          {/* Tabs */}
          <nav className="hidden sm:flex items-end gap-1 self-end pb-0">
            {TABS.map(tab => (
              <button key={tab.id} onClick={() => onTabChange(tab.id)}
                className={`relative px-5 py-2.5 text-sm font-semibold rounded-t-xl transition-all duration-150 flex items-center gap-1.5
                  ${activeTab === tab.id
