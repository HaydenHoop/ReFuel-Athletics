...
function buildFormula(answers) {
  ...
  // Carbs — base from duration, then scaled by weight
  // Science: ~0.5–1g carb/kg/hr is the effective range
  ...
  // Heavier athletes get a small bump, lighter athletes a small reduction
  const weightMultiplier = weightKg / 70; // 70kg is baseline
  const carbs = Math.round(Math.min(90, Math.max(15, durationBase * weightMultiplier)));

  // Sodium — sweat rate + weight (bigger body = more total sweat loss)
  ...
  const sodiumWeightBump = Math.round((weightKg - 70) * 1.5); // ~1.5mg extra per kg over baseline
  const sodium = Math.min(700, Math.max(50, sodiumBase + sodiumWeightBump));

  // Caffeine — dosed at ~1mg/kg for moderate effect, 0 if they don't want it
  const caffeineDose = {
    'Yes — every dose':   Math.round(Math.min(150, weightKg * 1.0)),
    'Yes — race day only': Math.round(Math.min(120, weightKg * 0.8)),
    'No thanks': 0,
  };
  const caffeine = caffeineDose[answers.caffeine] ?? 0;
  ...
  // Gut sensitivity → fructose ratio
